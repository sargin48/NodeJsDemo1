module.exports = function(app, db, moment, myService ) {
    
    Date.prototype.nextSecondDay = (day_of_week, weekIndexInMonth) =>{
    ...
    app.get('/schedule/:occurance(\\d+)/:every(\\d+)/:starting_date/:index(\\d+)/:day_of_week(\\d+)', (req, res) => {
    ...
    app.get('/v2/schedule/:occurance(\\d+)/:every(\\d+)/:starting_date/:index(\\d+)/:day_of_week(\\d+)', (req, res) => {
        ...
        let startDateCopy = moment(startDate).clone().toDate()
        let countWeekDayOccurencesInMonth = myService.countWeekdayOccurrencesInMonth(startDateCopy)
